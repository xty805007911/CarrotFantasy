...
public class Star extends Towers{
		...
		try{
			for(int i=0;i<TowerImages.length;i++) {
				for(int j=0;j<TowerImages[i].length;j++) {
					TowerImages[i][j]=ImageIO.read(Star.class.getResource("../png/Star"+i+j+".png"));
				}
			}
			TowerBase=ImageIO.read(Star.class.getResource("../png/StarBase.png"));
		}catch(Exception e){
			e.printStackTrace();
	...
	public Bullets getNewBullet() {
		if(shootState) {
			if(index++%15==0) {
				return (new BulletOfStar(this.x, this.y, this.towerAngrad, this.level,this.cannon_shot,this.ATK));	
			}
		}
		return null;
	}
